    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading">
            <h2>
                ...
                Agendamento <b>Massagem nos Pés</b>
            </h2>
            <small>
                São {{$terapia['tempo_sessao']}} minutos por sessão. Saiba mais sobre Massagem nos Pés <a href="https://www.google.com/search?q=Massagem+nos+Pés" target="_blank">aqui</a>. Sessões toda quarta-feira.<br/>
                <b>O limite de agendamentos por usuário é de {{$terapia['limite_mensal']}} sessão por mês.</b>
            </small>
        </div>
        ...
        <div class="panel-body">
            ...
            <div class="row">
                @foreach(array_chunk($massagem_pes_array['dias'], 1) as $chunk_dias)
                                                            ...
                                                            <td width="60%" style="padding:0;padding-top:3px;">
                                                                ...
                                                                @if($horario) {{-- se horário ocupado --}}
                                                                    ...
                                                                    @if($horario == Auth::user()->name)
                                                                        <form class="form-horizontal" method="POST" action="{{action('Terapias\TerapiasController@cancelar_massagem_pes')}}">
                                                                            {{csrf_field()}}
                                                                ...
                                                                @else {{-- se horário livre --}}
                                                                    ...
                                                                    @else {{-- senão passado --}}
                                                                        ...
                                                                        @if($dia['limite_mensal'] || $dia['limite_diario'] || $dia['limite_intervalo_agendamento']) {{-- se já bateu limite mensal, diário (outras terapias incluso) ou intervalo agendamento --}}
                                                                            ...
                                                                            @if(Carbon\Carbon::now()->between(Carbon\Carbon::parse($dia['dia']." ".$key)->subMinutes($limite_livre_bonus), Carbon\Carbon::parse($dia['dia']." ".$key))) {{-- se ainda livre 30 min antes do início --}}
                                                                                <form class="form-horizontal" method="POST" action="{{action('Terapias\TerapiasController@agendar_massagem_pes')}}">
                                                                                    {{csrf_field()}}
                                                                                    <input name="data" type="hidden" value="{{Carbon\Carbon::parse($dia['dia'])->format('Y-m-d')}}">
                                                                                    <input name="hora" type="hidden" value="{{$key}}:00">
